...
def summon_global_3d_lulu():
    """全地形无敌装甲版：纯净国内 CDN (淘宝NPM) + 防白屏兜底 + 绝对凝视"""
    ...
    <script>
        ...
        pWin.__LULU_V12_INIT = true; 

        const loadScript = (src) => new Promise((res) => {{
            const s = pDoc.createElement('script');
            s.src = src; s.onload = res; s.onerror = () => {{ console.error("加载脚本失败: " + src); res(); }};
            pDoc.head.appendChild(s);
        }});

        const initLulu = async () => {{
            if (!pWin.THREE || !pWin.THREE.DRACOLoader) {{
                // 🔥 终极杀手锏：彻底抛弃海外所有 CDN，全部使用淘宝极速镜像，确保网络 100% 畅通 🔥
            ...
            script.innerHTML = `
                (function() {{
                    ...
                    // 🔥 更换 Draco 解码器的地址为淘宝 NPM 镜像，彻底解决“解码卡死”问题 🔥
                    ...
                    const switchModel = (b64String, name) => {{
                        ...
                        // 设置一个强制超时器，如果 10 秒还卡着，强行断开并提醒
                        const loadTimeout = setTimeout(() => {{
                            bubble.innerText = "❌ 解码超时！请提供未加密的通用 .glb 模型";
                        ...
                        loader.load(
                            ...
                            (gltf) => {{
                                clearTimeout(loadTimeout); // 解除超时报警
...
def render_futures_backtest():
    ...
    st.markdown(
        '<div class="glass-card"><h3 style="margin-bottom:0;">🔗 期货全量审计与归因分析</h3><p class="sub-text">已切换至全免费无限制的 AkShare 开源数据引擎。直接输入代码，自动拉取分钟与日线数据！</p></div>',
        unsafe_allow_html=True)
    ...
    with c2:
        ...
        if st.session_state.fut_bt_data is not None:
            ...
            c1.markdown(
                f'<div class="metric-box"><p>累计收益 (双边多空计算)</p><h2 style="color:#3b82f6;">{m["total"] * 100:.2f}%</h2></div>',
                unsafe_allow_html=True)
            c2.markdown(
                f'<div class="metric-box"><p>期末总权益</p><h2 style="color:#3b82f6;">¥ {m["init_cash"] * (1 + m["total"]):,.0f}</h2></div>',
                unsafe_allow_html=True)
            c3.markdown(
                f'<div class="metric-box"><p>最大资金回撤</p><h2 style="color:#ef4444;">{m["max_dd"] * 100:.2f}%</h2></div>',
                unsafe_allow_html=True)
            c4.markdown(
                f'<div class="metric-box"><p>最高保证金占用</p><h2 style="color:#3b82f6;">¥ {m["max_margin"]:,.0f}</h2></div>',
                unsafe_allow_html=True)
...
def render_futures_sandbox():
    st.markdown(
        '<div class="glass-card"><h3 style="margin-bottom:0;">🌪️ 期货高频沙盘模拟推演</h3><p class="sub-text">Tick 级盘口模拟、毫秒级信号响应测试与动态滑点侦测。</p></div>',
        unsafe_allow_html=True)
    ...
    if is_running:
        ...
        while is_running:
            ...
            with chart_placeholder.container():
                fig = go.Figure(data=go.Scatter(
                    y=tick_history, mode='lines', line=dict(color='#3b82f6', width=2),
                    fill='tozeroy', fillcolor='rgba(59, 130, 246, 0.1)'
                ))
                fig.update_layout(
                    height=380, template="none", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
        ...
        <div class="metric-box" style="height: 380px; display: flex; flex-direction: column; justify-content: center; align-items: center;">
            <p>高频推演</p><h2 style="color: #3b82f6;">等待引擎唤醒...</h2>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_r:
        if st.session_state.dl_result:
            ...
            with st.expander("🤖 AI 深度预测白盒解析舱 (点击展开/收起)", expanded=True):
                st.markdown(
                    f"**📈 极速解盘预览**：当前实盘价 `<span style='color:#3b82f6;'>{latest_price:.2f}</span>` | 驱动核心: {model_desc}",
                    unsafe_allow_html=True)
