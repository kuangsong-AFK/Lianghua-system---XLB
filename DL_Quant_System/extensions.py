...
if "fut_bt_result" not in st.session_state: st.session_state.fut_bt_result = None
...
st.markdown(f"<style>.block-container {{ animation: {anim_name} 0.65s cubic-bezier(0.2, 0.8, 0.2, 1) forwards; background: transparent !important; padding-top: 4.5rem !important; padding-bottom: 120px !important; }}</style>", unsafe_allow_html=True)
...
def execute_safely(code, df):
    ...
    df_ai['Signal'] = df_ai[sig_col].fillna(0).apply(lambda x: 1 if x > 0.1 else (-1 if x < -0.1 else 0)).astype(int) if sig_col else 0
    return df_ai
...
def render_smart_charts(df):
    ...
    fig = make_subplots(rows=rows, cols=1, shared_xaxes=True, vertical_spacing=0.03, row_heights=[0.5, 0.15] + [0.35 / max(1, len(sub_groups))] * len(sub_groups))
    fig.add_trace(go.Candlestick(x=df['trade_date'], open=df['Open'], high=df['High'], low=df['Low'], close=df['Close'], increasing_line_color='#FD1050', decreasing_line_color='#00FF00', name='K线'), row=1, col=1)
    colors = ['#FFFF00', '#FF00FF', '#00FFFF', '#FFFFFF']
    for i, col in enumerate(main_inds): fig.add_trace(go.Scatter(x=df['trade_date'], y=df[col], name=col, line=dict(width=1.2, color=colors[i % 4])), row=1, col=1)
    if 'Signal' in df.columns:
        buys, sells = df[df['Signal'] == 1], df[df['Signal'] == -1]
        fig.add_trace(go.Scatter(x=buys['trade_date'], y=buys['Low'] * 0.95, mode='markers', marker=dict(symbol='triangle-up', size=14, color='#00FFFF'), name='买'), row=1, col=1)
        fig.add_trace(go.Scatter(x=sells['trade_date'], y=sells['High'] * 1.05, mode='markers', marker=dict(symbol='triangle-down', size=14, color='#FF00FF'), name='卖'), row=1, col=1)
    fig.add_trace(go.Bar(x=df['trade_date'], y=df.get('Volume', np.zeros(len(df))), marker_color=np.where(df['Close'] >= df['Open'], '#FD1050', '#00FF00'), name='成交量'), row=2, col=1)

    row_idx = 3
    for gid in sorted(sub_groups.keys(), key=int):
        for i, col in enumerate(sub_groups[gid]):
            if 'HIST' in col.upper():
                fig.add_trace(go.Bar(x=df['trade_date'], y=df[col], marker_color=np.where(df[col] >= 0, '#FD1050', '#00FF00'), name=col), row=row_idx, col=1)
            else:
                fig.add_trace(go.Scatter(x=df['trade_date'], y=df[col], line=dict(width=1.2, color=colors[i % 4]), name=col), row=row_idx, col=1)
        row_idx += 1

    fig.update_layout(height=500 + len(sub_groups) * 150, template="none", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', xaxis_rangeslider_visible=False, dragmode='pan', hovermode='x', showlegend=False)
...
if selected_page == PAGES[0]:
    st.markdown('<div class="glass-card"><h1 style="margin-bottom:0; color:var(--text-color);">🏛️ 全链路智能量化决策枢纽</h1><p class="highlight-text" style="font-size:1.1rem; margin-top:5px;">System Overview & Mid-term Inspection Dashboard</p></div>', unsafe_allow_html=True)
    c1, c2, c3, c4 = st.columns(4)
    with c1: st.metric("活跃并发沙盒 (UUID)", st.session_state.user_id)
    with c2: st.metric("Tushare 行情链路", get_tushare_status())
    with c3: st.metric("大模型底层通信", "🟢 Moonshot-v1 正常")
    with c4: st.metric("AI 神经网络", "🟢 融合学习待命")
    st.markdown("---")
    ...
    with c_arch:
        ...
        """, unsafe_allow_html=True)
    with c_point:
        st.markdown('<div class="glass-card"><h4 style="color:var(--text-color);">📋 平台监控与杀手锏</h4>**云端依赖环境**<br>🟢 requirements.txt 托管<br><br>**答辩核心创新点：**<br>✅ 原生一键直连上传<br>✅ 变量作用域防塌陷<br>✅ 全栈语义防截断</div>', unsafe_allow_html=True)

elif selected_page == PAGES[1]:
    st.markdown('<div class="glass-card"><h3 style="margin-bottom:0; color:var(--text-color);">🤖 LLM 策略战情室</h3><p class="sub-text">多模态视觉引擎与全域文档解析模块已就绪，体验沉浸式工作流。</p></div>', unsafe_allow_html=True)
    ctrl_col1, ctrl_col2 = st.columns([1, 1])
    with ctrl_col1: selected_model = st.selectbox("🧠 选择大模型算力通道", ["moonshot-v1-8k", "moonshot-v1-32k", "moonshot-v1-128k"], index=0)
    with ctrl_col2: st.markdown("<div style='height: 32px;'></div>", unsafe_allow_html=True); enable_deep_think = st.toggle("💡 强子注入：开启深度思考引擎 (CoT)", value=False)
    ...
    uploaded_files = st.file_uploader("选择文件", accept_multiple_files=True, type=['pdf', 'doc', 'docx', 'csv', 'txt', 'png', 'jpg', 'jpeg'], label_visibility="collapsed")
    file_context_text = ""
            ...
            with cols[idx % 3]:
                ...
                if file.type.startswith('image/'):
                    ...
                    if PyPDF2:
                        ...
                        except Exception as e: st.error(f"PDF 读取异常: {e}")
                    else: st.info(f"💡 提示：检测到 PDF 文件，请在 requirements.txt 中添加 PyPDF2。")
                    ...
                    if docx:
                        ...
                        except Exception as e: st.error(f"Word 读取异常: {e}")
                    else: st.info(f"💡 提示：检测到 Word 文件，请在 requirements.txt 中添加 python-docx。")
    ...
    if raw_prompt := st.chat_input("向小吕布量化架构师发送军令..."):
        ...
        with chat_container:
            with st.chat_message("user"): st.markdown(raw_prompt)
            with st.chat_message("assistant"):
                ...
                1. **沙盒限制**：你只能使用 pandas (pd), numpy (np) 和 math。绝对禁止 `import talib`！所有技术指标必须用 pandas 手搓。
                2. **数据源字段**：传入的 df 包含：['Open', 'High', 'Low', 'Close', 'Volume']。
                3. **画图命名协议**：主图指标必须以 `MAIN_` 开头，副图指标必须以 `SUB1_` 或 `SUB2_` 开头。
                4. **交易信号协议**：必须生成一列 `df['Signal']`。1=买入，-1=卖出，0=持有/空仓。
                5. **代码骨架**：
                {ticks}python
                ...
                {ticks}
                请直接输出代码及策略白话解析。"""
                messages_to_send = [{"role": "system", "content": sys_p}] + st.session_state.messages[:-1] + [{"role": "user", "content": full_prompt_for_ai}]
                max_retries, agent_logs, last_error, full_resp = 2, [], "", ""
                msg_box = st.empty()

                for attempt in range(max_retries + 1):
                    if attempt > 0:
                        agent_logs.append(f'<div class="agent-status-node retry">🔄 <b>尝试 {attempt}:</b> 沙盒拦截异常 (<code>{last_error}</code>) -> Agent 发起重构</div>')
                        messages_to_send.extend([{"role": "assistant", "content": full_resp}, {"role": "user", "content": f"代码报错：`{last_error}`，请严格遵循模板修复。"}])
                    try:
                        stream = client.chat.completions.create(model=selected_model, messages=messages_to_send, stream=True, temperature=0.3 if enable_deep_think else 0.7)
                        full_resp = ""
                        for chunk in stream:
                            if chunk.choices[0].delta.content:
                                full_resp += chunk.choices[0].delta.content
                                msg_box.markdown(full_resp.replace("<think>", "🧠 深度思考中...\n\n").replace("</think>", "\n\n---\n") + "▌", unsafe_allow_html=True)
                        msg_box.markdown(full_resp.replace("<think>", "🧠 深度思考过程：\n").replace("</think>", "\n---\n"), unsafe_allow_html=True)

                        code_match = re.search(r"`{3}python\s*(.*?)\s*`{3}", full_resp, re.DOTALL)
                        resp_clean = re.sub(r"<think>.*?</think>", "", full_resp, flags=re.DOTALL)
                        explanation = re.sub(r"`{3}python\s*.*?\s*`{3}", "", resp_clean, flags=re.DOTALL).strip().replace("【策略白话解析】", "").strip()

                        if explanation: st.session_state.strategy_explanation = explanation
                        else: st.session_state.strategy_explanation = "该策略完全由硬核代码驱动，未返回额外人话分析。"

                        if not code_match: break

                        extracted_code = code_match.group(1).strip()
                        try:
                            dummy_df = pd.DataFrame({'trade_date': pd.date_range('20230101', periods=50), 'Open': np.random.rand(50) * 10, 'High': np.random.rand(50) * 12, 'Low': np.random.rand(50) * 8, 'Close': np.random.rand(50) * 10})
                            ...
                            agent_logs.append(f'<div class="agent-status-node success">✅ <b>尝试 {attempt + 1}:</b> 代码通过沙盒预检 -> 策略已安全装载</div>')
                            st.markdown("".join(agent_logs), unsafe_allow_html=True)
                            break
                        except Exception as e:
                            last_error = str(e)
                            if attempt == max_retries:
                                agent_logs.append(f'<div class="agent-status-node error">❌ <b>最终结果:</b> 失败，最终报错: <code>{last_error}</code></div>')
                                st.markdown("".join(agent_logs), unsafe_allow_html=True)
                    except Exception as e:
                        st.error(f"链路断开: {e}")
                        full_resp += f"\n\n❌ [异常阻断: 通信失败或超载 - {e}]"
    ...
    st.markdown('<div class="glass-card"><h3 style="color:var(--text-color); margin-bottom:0;">📊 历史回测全量审计与归因分析</h3></div>', unsafe_allow_html=True)
    ...
    with col_r:
        if st.session_state.bt_result:
            ...
            c1, c2, c3, c4 = st.columns(4)
            c1.markdown(f'<div class="metric-box"><p>累计收益</p><h2 class="highlight-text">{m["total"] * 100:.2f}%</h2></div>', unsafe_allow_html=True)
            c2.markdown(f'<div class="metric-box"><p>年化收益</p><h2 class="highlight-text">{m["annual"] * 100:.2f}%</h2></div>', unsafe_allow_html=True)
            c3.markdown(f'<div class="metric-box"><p>最大回撤</p><h2 class="danger-text">{m["max_dd"] * 100:.2f}%</h2></div>', unsafe_allow_html=True)
            c4.markdown(f'<div class="metric-box"><p>夏普比率</p><h2 class="highlight-text">{m["sharpe"]:.2f}</h2></div>', unsafe_allow_html=True)
            ...
            if st.session_state.generated_code and st.session_state.strategy_explanation != "暂无策略解析，请先前往 AI 战情室下达军令。":
                with st.expander("💡 展开：AI 策略白话解析", expanded=False): st.markdown(st.session_state.strategy_explanation)
            st.plotly_chart(render_smart_charts(df), use_container_width=True, config={'scrollZoom': True})

elif selected_page == PAGES[3]:
    st.markdown('<div class="glass-card"><h3 style="color:var(--text-color); margin-bottom:0;">⚡ 高频沙盘模拟推演 (Real-time Flow)</h3></div>', unsafe_allow_html=True)
    c_ctrl, c_chart = st.columns([1, 2.5])
    with c_ctrl:
        live_code = st.text_input("🎯 动态推送标的", value="000001")
        freq = st.slider("⏱️ 刷新间隔 (秒)", 0.1, 2.0, 0.5)
        st.button("▶️ 开启高频推演", on_click=lambda: st.session_state.update({"is_live_trading": True}), type="primary")
        st.button("⏹️ 强行停止", on_click=lambda: st.session_state.update({"is_live_trading": False}))
    with c_chart:
        if st.session_state.generated_code and st.session_state.strategy_explanation != "暂无策略解析，请先前往 AI 战情室下达军令。":
            with st.expander("💡 当前军令：策略白话解析", expanded=False): st.markdown(st.session_state.strategy_explanation)
        ...
        if st.session_state.is_live_trading:
            ...
            for i in range(20, len(stream)):
                ...
                except Exception as e: st.error(f"高频熔断: {e}"); st.session_state.is_live_trading = False; break
                time.sleep(freq)
    ...
    with col_l:
        ...
        if st.button("🚀 启动张量训练", type="primary", use_container_width=True):
            if not model_choices: st.error("主公，请至少选择一种预测模型！")
            else:
                with st.spinner("神经网络前向传播中..."):
                    try:
                        ...
                        class LSTM_Model(nn.Module):
                            def __init__(self): super().__init__(); self.lstm = nn.LSTM(1, 64, 2, batch_first=True); self.fc = nn.Linear(64, 1)
                            def forward(self, x): out, _ = self.lstm(x); return self.fc(out[:, -1, :])

                        class GRU_Model(nn.Module):
                            def __init__(self): super().__init__(); self.gru = nn.GRU(1, 64, 2, batch_first=True); self.fc = nn.Linear(64, 1)
                            def forward(self, x): out, _ = self.gru(x); return self.fc(out[:, -1, :])

                        class CNN_1D_Model(nn.Module):
                            def __init__(self, seq_len): super().__init__(); self.conv = nn.Conv1d(in_channels=1, out_channels=32, kernel_size=3, padding=1); self.fc = nn.Linear(32 * seq_len, 1)
                            def forward(self, x): x = x.permute(0, 2, 1); x = torch.relu(self.conv(x)); x = x.reshape(x.size(0), -1); return self.fc(x)
                        ...
                        for m_idx, m_name in enumerate(model_choices):
                            ...
                            model = LSTM_Model() if m_name == "LSTM" else GRU_Model() if m_name == "GRU" else CNN_1D_Model(slen)
                        ...
                        st.session_state.dl_result = {"dates": df['trade_date'].iloc[-100:], "actual": df['Close'].iloc[-100:], "preds": preds_dict, "future": future_preds_dict, "models_used": model_choices}
                    except Exception as e: st.error(f"DL 张量异常: {e}")
    ...
    with col_r:
        if st.session_state.dl_result:
            ...
            for m_name, pred_array in res['preds'].items(): fig.add_trace(go.Scatter(x=res['dates'], y=pred_array, name=f'{m_name} 历史拟合', line=dict(color=color_map.get(m_name, '#ffffff'), dash='dot', width=1)))
            if len(res['preds']) > 1: fig.add_trace(go.Scatter(x=res['dates'], y=np.mean(list(res['preds'].values()), axis=0), name='🔥 均值集成 (Ensemble)', line=dict(color='#ff4b4b', width=3)))
    ...
    with c1:
        if os.path.exists("user_logs/global_master_log.csv"): st.download_button("📁 导出审计日志", data=pd.read_csv("user_logs/global_master_log.csv").to_csv(index=False).encode('utf-8'), file_name='Audit_Logs.csv', type="primary")
    with c2:
