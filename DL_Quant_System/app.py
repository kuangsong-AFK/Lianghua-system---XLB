...
if selected_page == PAGES[0]:
    ...
    with c4:
        st.metric("扩展引擎状态", "🔴 异常" if extensions_err else "🟢 已安全挂载")
    ...
    if extensions:
        ...
        st.error("🚨 扩展模块加载失败：请检查 GitHub 的 `extensions.py` 文件是否已上传。")
    ...
    if extensions:
        ...
        st.error("🚨 请检查 GitHub，确保 `extensions.py` 文件已正确上传。")
