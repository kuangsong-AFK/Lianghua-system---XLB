...
if "strategy_explanation" not in st.session_state:
    st.session_state.strategy_explanation = "暂无策略解析，请先前往 AI 战情室下达军令。"
...
if "prev_page" not in st.session_state:
    st.session_state.prev_page = PAGES[0]
if "just_switched" not in st.session_state:
...
# 🛡️ 撤销害人的 Python 级内存锁，回归最稳健的渲染模式！让 JS 自己管自己！
components.html(f"""
<script>
    {scroll_script}
        ...
        requestAnimationFrame(() => {{
            ...
            if (chatInputOuter && fileInput) {{
                ...
                if (innerPill && !doc.getElementById('fake-attach-btn')) {{
                    ...
                    fakeBtn.style.cssText = 'position: absolute !important; left: 16px !important; top: 50% !important; transform: translateY(-50%) !important; z-index: 9999 !important; display: flex; align-items: center; justify-content: center; width: 24px; height: 24px; cursor: pointer;';
...
# 🛡️ 撤销害人的 Python 级内存锁，保证包含桌宠渲染循环的 iframe 永不被销毁！
if extensions:
...
if selected_page == PAGES[0]:
    ...
    with c_point:
        st.markdown(
            '<div class="glass-card"><h4 style="color:var(--text-color);">📋 平台监控与杀手锏</h4>**云端依赖环境**<br>🟢 requirements.txt 托管<br><br>**答辩核心创新点：**<br>✅ 原生一键直连上传<br>✅ 变量作用域防塌陷<br>✅ 局部 Fragment 零闪烁技术</div>',
            unsafe_allow_html=True
        )
