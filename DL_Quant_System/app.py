...
# ==========================================
PAGES = [
    "🏠 系统总览 (监控中控)", "🤖 AI 策略引擎 (LLM)", "💻 极客量化 IDE (代码编译)",
    "📈 深度静态全量回测", "⚡ 实时高频交易 (Live)", "🧠 深度学习预测矩阵",
    "🛡️ 论文审计日志", "🔗 期货全量审计 (归因)", "🌪️ 期货高频沙盘", "🧩 扩展插件中心"
]
...
if "core_ui_injected" not in st.session_state:
    ...
    <script>
        ...
        window.parent.__UI_OBSERVER.observe(window.parent.document.body, {{ childList: true, subtree: true }});
    </script>
    """, height=0, width=0)
...
st.markdown("""
<style>
    /* 去除了所有强制的背景颜色和文字颜色，完全交给 Streamlit 原生主题控制 */
    header[data-testid="stHeader"] { background: transparent !important; }

    /* 半透明中性玻璃态卡片，在浅色和深色主题下都很好看 */
    .glass-card { 
        background: rgba(128, 130, 135, 0.08) !important; 
        backdrop-filter: blur(12px); 
        border: 1px solid rgba(128, 130, 135, 0.2); 
        border-radius: 16px; 
        padding: 25px; 
        margin-bottom: 20px; 
    ...
    [data-testid="stChatInput"] > div:first-child { 
        background-color: rgba(128, 130, 135, 0.1) !important; 
        backdrop-filter: blur(20px) !important; 
        border: 1px solid rgba(128, 130, 135, 0.3) !important; 
...
if selected_page == PAGES[0]:
    st.markdown(
        '<div class="glass-card"><h1 style="margin-bottom:0;">🏛️ 全链路智能量化决策枢纽</h1><p style="font-size:1.1rem; margin-top:5px;">System Overview Dashboard</p></div>',
        unsafe_allow_html=True)
    ...
    with c_point:
        st.markdown(
            '<div class="glass-card"><h4>📋 平台监控与杀手锏</h4>**云端依赖环境**<br>🟢 requirements.txt 托管<br><br>**核心架构升级：**<br>✅ <b>完美适配明亮/暗黑双主题</b><br>✅ 前端引擎防抖极速化<br>✅ <b>代码沙盒全域防爆盾</b><br>✅ <b>淘宝 NPM 极速解码通道</b></div>',
            unsafe_allow_html=True
        )

elif selected_page == PAGES[1]:
    st.markdown(
        '<div class="glass-card"><h3 style="margin-bottom:0;">🤖 LLM 策略战情室</h3><p>多模态视觉引擎与全域文档解析模块已就绪，体验沉浸式工作流。</p></div>',
        unsafe_allow_html=True)
                ...
                with st.spinner("神经网络前向传播中..."):
                    try:
                        ...
                        class CNN_1D_Model(nn.Module):
                            ...
                            def forward(self, x): x = torch.relu(self.conv(x.permute(0, 2, 1))); return self.fc(
    ...
    with col_r:
        if st.session_state.dl_result:
            ...
            with st.expander("🤖 AI 深度预测白盒解析舱 (点击展开/收起)", expanded=True):
                st.markdown(f"**📈 极速解盘预览**：当前实盘价 `{latest_price:.2f}` | 驱动核心: {model_desc}")
                c_f1, c_f2, c_f3, c_f4 = st.columns(4)
                c_f1.metric("未来 1 天预测", f"{day1_pred:.2f}",
                            f"{(day1_pred - latest_price) / latest_price * 100:.2f}%")
                c_f2.metric("未来 5 天预测", f"{day5_pred:.2f}",
                ...
                if st.button("✨ 召唤 Kimi 结合胜率生成人话解盘", use_container_width=True):
                    ai_ph = st.empty()
                    prompt = f"你是一个顶级的量化分析师，专门为小白用户提供白话解盘。已知某标的当前收盘价为 {latest_price:.2f}元。基于【{model_desc}】预测价为 {day1_pred:.2f}元，未来5天为 {day5_pred:.2f}元。历史胜率 {success_rate:.1f}%，绝对偏差 {mape:.2f}%。请用大白话（限200字以内），向小白解释并给出建议。"
                    try:
