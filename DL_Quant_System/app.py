...
# 🔥 核心防御：捕获一切导入异常，保护主系统不崩溃！ 🔥
extensions_err = None
try:
    import extensions
except Exception as e:
    extensions = None
    extensions_err = f"{type(e).__name__}: {str(e)}"
...
def get_ai_client():
    ...
    return OpenAI(api_key=KIMI_API_KEY, base_url="https://api.moonshot.cn/v1", timeout=60.0)
...
# ==========================================
# 2. 静态 CSS 护甲 (保留您的高级流光主题)
# ==========================================
st.markdown("""
<style>
    @keyframes fluidFlow { 0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; } }
    .stApp { background: linear-gradient(132deg, #040914, #0b193d, #06112a, #11224d) !important; background-size: 400% 400% !important; animation: fluidFlow 20s ease infinite !important; }
    ...
    .glass-card { background: rgba(15, 23, 42, 0.6) !important; backdrop-filter: blur(16px) !important; border: 1px solid rgba(255, 255, 255, 0.1) !important; border-radius: 16px; padding: 25px; margin-bottom: 20px; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.5); }
...
with st.sidebar:
    ...
    if extensions and hasattr(extensions, 'summon_sidebar_3d_lulu'):
...
def get_tushare_status():
    try:
        get_ts_pro().trade_cal(exchange='SSE', start_date='20240101', end_date='20240101');
        return "🟢 Online"
    except:
        return "🔴 Offline"
...
def fetch_and_clean_data(ts_code, adj, start_date):
    df = get_ts_pro().pro_bar(ts_code=ts_code, adj=adj, start_date=start_date)
...
if selected_page == PAGES[0]:
    if extensions_err:
        st.error(
            f"🚨 **代码加载防御系统启动** 🚨\n\n检测到 `extensions.py` 文件中存在语法错误或导入异常：\n\n`{extensions_err}`\n\n但这没有关系，主系统防爆盾已拦截崩溃，您可以正常使用其他功能。如果您刚进行了粘贴，请确保复制完整！")
            ...
            with st.chat_message("assistant"):
                ...
                for attempt in range(max_retries + 1):
                    ...
                    try:
                        ...
                        stream = ai_client.chat.completions.create(model=selected_model, messages=valid_msgs,
    ...
    if extensions:
        ...
        st.error("🚨 请检查 GitHub，确保 `extensions.py` 文件已正确上传且无语法错误。")
    ...
    if custom_plugins and hasattr(custom_plugins, 'route_and_render'): custom_plugins.route_and_render(selected_page)import os
import sys
import uuid
import math
import time
...
try: import custom_plugins
except Exception: custom_plugins = None
...
for key, val in {"user_id": f"User_{str(uuid.uuid4())[:6]}", "messages": [], "generated_code": "", "strategy_explanation": "暂无策略解析。", "dl_result": None, "bt_result": None, "sys_logs": [], "is_live_trading": False}.items():
    if key not in st.session_state: st.session_state[key] = val
...
# 3. 页面路由分配
# ==========================================
PAGES = ["🏠 系统总览 (监控中控)", "🤖 AI 策略引擎 (LLM)", "💻 极客量化 IDE (代码编译)", "📈 深度静态全量回测", "⚡ 实时高频交易 (Live)", "🧠 深度学习预测矩阵", "🛡️ 论文审计日志", "🔗 期货全量审计 (归因)", "🌪️ 期货高频沙盘", "🧩 扩展插件中心"]
if custom_plugins and hasattr(custom_plugins, 'EXTRA_PAGES'): PAGES.extend(custom_plugins.EXTRA_PAGES)
...
def get_tushare_status():
    try:
        get_ts_pro().trade_cal(exchange='SSE', start_date='20240101', end_date='20240101'); return "🟢 Online"
...
def execute_safely(code, df):
    ...
    try:
        ...
        if df_ai is None or not hasattr(df_ai, 'columns'): return df
        sig_col = next((c for c in df_ai.columns if c.lower() == 'signal'), None)
        if sig_col: df_ai['Signal'] = df_ai[sig_col].fillna(0).apply(lambda x: 1 if x > 0.1 else (-1 if x < -0.1 else 0)).astype(int)
        else: df_ai['Signal'] = 0
        return df_ai
...
def render_smart_charts(df):
    ...
    x_labels = df['trade_date'].dt.strftime('%Y-%m-%d') if df['trade_date'].dt.time.nunique() <= 1 else df['trade_date'].dt.strftime('%m-%d %H:%M')

    fig.add_trace(go.Candlestick(x=x_labels, open=df['Open'], high=df['High'], low=df['Low'], close=df['Close'], increasing_line_color='#ef4444', decreasing_line_color='#10b981', name='K线'), row=1, col=1)
    colors = ['#0ea5e9', '#f59e0b', '#8b5cf6', '#ec4899']
    for i, col in enumerate(main_inds): fig.add_trace(go.Scatter(x=x_labels, y=df[col], name=col, line=dict(width=1.2, color=colors[i % 4])), row=1, col=1)
    if 'Signal' in df.columns:
        buys = df[df['Signal'] == 1]; sells = df[df['Signal'] == -1]
        buy_x = buys['trade_date'].dt.strftime('%Y-%m-%d' if df['trade_date'].dt.time.nunique() <= 1 else '%m-%d %H:%M')
        sell_x = sells['trade_date'].dt.strftime('%Y-%m-%d' if df['trade_date'].dt.time.nunique() <= 1 else '%m-%d %H:%M')
        fig.add_trace(go.Scatter(x=buy_x, y=buys['Low'] * 0.998, mode='markers', marker=dict(symbol='triangle-up', size=14, color='#ef4444'), name='买'), row=1, col=1)
        fig.add_trace(go.Scatter(x=sell_x, y=sells['High'] * 1.002, mode='markers', marker=dict(symbol='triangle-down', size=14, color='#10b981'), name='卖'), row=1, col=1)
    fig.add_trace(go.Bar(x=x_labels, y=df.get('Volume', np.zeros(len(df))), marker_color=np.where(df['Close'] >= df['Open'], '#ef4444', '#10b981'), name='成交量'), row=2, col=1)
    row_idx = 3
    for gid in sorted(sub_groups.keys(), key=int):
        for i, col in enumerate(sub_groups[gid]):
            if 'HIST' in col.upper(): fig.add_trace(go.Bar(x=x_labels, y=df[col], marker_color=np.where(df[col] >= 0, '#ef4444', '#10b981'), name=col), row=row_idx, col=1)
            else: fig.add_trace(go.Scatter(x=x_labels, y=df[col], line=dict(width=1.5, color=colors[i % 4]), name=col), row=row_idx, col=1)
        row_idx += 1

    fig.update_layout(height=500 + len(sub_groups) * 150, template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', xaxis_rangeslider_visible=False, dragmode='pan', hovermode='x', showlegend=False, margin=dict(l=10, r=10, t=10, b=10))
    fig.update_xaxes(type='category', categoryorder='array', categoryarray=x_labels, nticks=8, showgrid=True, gridwidth=1, gridcolor='rgba(255,255,255,0.05)')
    fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor='rgba(255,255,255,0.05)')
    return fig
...
if selected_page == PAGES[0]:
    if extensions_err:
        st.error(f"🚨 **代码加载防御系统启动** 🚨\n\n检测到 `extensions.py` 文件中存在语法错误或导入异常：\n\n`{extensions_err}`\n\n但这没有关系，主系统防爆盾已拦截崩溃，您可以正常使用其他功能。如果您刚进行了粘贴，请确保复制完整！")

    st.markdown('<div class="glass-card"><h1 style="margin-bottom:0;">🏛️ 全链路智能量化决策枢纽</h1><p class="highlight-text" style="font-size:1.1rem; margin-top:5px;">System Overview Dashboard</p></div>', unsafe_allow_html=True)
    c1, c2, c3, c4 = st.columns(4)
    ...
    with c4: st.metric("扩展引擎状态", "🔴 存在错误" if extensions_err else "🟢 已安全挂载")
    ...
    st.markdown('<style>div[data-testid="stFileUploader"] { margin-top: -20px; }</style>', unsafe_allow_html=True)
    st.markdown('<div class="glass-card"><h3 style="margin-bottom:0;">🤖 LLM 策略战情室</h3><p class="sub-text">极速云端直连版本，去除了多余前端监控，享受最纯粹的 AI 算力。</p></div>', unsafe_allow_html=True)
    ...
    uploaded_files = st.file_uploader("📎 附件上传 (自动提取数据)", accept_multiple_files=True, type=['pdf', 'doc', 'docx', 'csv', 'txt'])
    file_context_text = ""
    if 'uploaded_files' in locals() and uploaded_files:
        for file in uploaded_files:
            fname_lower = file.name.lower()
    ...
    if raw_prompt := st.chat_input("向小吕布量化架构师发送军令..."):
        ...
        with chat_container:
            ...
            with st.chat_message("assistant"):
                ...
                max_retries = 2; agent_logs = []; last_error = ""; full_resp = ""; msg_box = st.empty()
                ai_client = get_ai_client()
                for attempt in range(max_retries + 1):
                    if attempt > 0:
                        agent_logs.append(f'<div class="agent-status-node retry">🔄 尝试 {attempt}: 沙盒拦截异常 ({last_error}) -> Agent 发起重构</div>')
                        safe_resp = full_resp if full_resp and full_resp.strip() else "(API 响应空)"
                        messages_to_send.extend([{"role": "assistant", "content": safe_resp}, {"role": "user", "content": f"代码报错：`{last_error}`，请修复。"}])
                    try:
                        valid_msgs = [m for m in messages_to_send if m.get("content") and str(m["content"]).strip()]
                        stream = ai_client.chat.completions.create(model=selected_model, messages=valid_msgs, stream=True, temperature=0.3)
                        full_resp = ""
                        for chunk in stream:
                            if chunk.choices[0].delta.content:
                                full_resp += chunk.choices[0].delta.content
                        ...
                        try:
                            ...
                            _ = execute_safely(extracted_code, add_default_indicators(dummy_df))
                            st.session_state.generated_code = extracted_code
                            agent_logs.append(f'<div class="agent-status-node success">✅ 尝试 {attempt + 1}: 预检通过，策略装载</div>')
                            st.markdown("".join(agent_logs), unsafe_allow_html=True); break
                        except Exception as e:
                            last_error = str(e)
                            if attempt == max_retries:
                                agent_logs.append(f'<div class="agent-status-node error">❌ 失败报错: {last_error}</div>')
                                st.markdown("".join(agent_logs), unsafe_allow_html=True)
                    except Exception as e:
                        st.error(f"链路断开: {e}"); full_resp += f"\n\n❌ [异常: {e}]"; break
                if not full_resp.strip(): full_resp = "❌ 大模型中断。"
                if agent_logs: full_resp += "\n\n" + "".join(agent_logs)
                st.session_state.messages.append({"role": "assistant", "content": full_resp})
        st.rerun()
    ...
    else: st.error("🚨 请检查 GitHub，确保 `extensions.py` 文件已正确上传且无语法错误。")

elif selected_page == PAGES[3]:
    st.markdown('<div class="glass-card"><h3 style="margin-bottom:0;">📊 历史回测全量审计与归因分析</h3></div>', unsafe_allow_html=True)
    col_l, col_r = st.columns([1, 3])
    with col_l:
        ts_code = format_ts_code(st.text_input("🎯 回测标的代码", value="000001"))
        span_mapping = {"近1年": 1, "近3年": 3, "近5年": 5, "近10年 (极限穿越)": 10}
    ...
    st.markdown('<div class="glass-card"><h3 style="margin-bottom:0;">⚡ 高频沙盘模拟推演 (Real-time Flow)</h3></div>', unsafe_allow_html=True)
    c_ctrl, c_chart = st.columns([1, 2.5])
    with c_ctrl:
        live_code = st.text_input("🎯 动态推送标的", value="000001")
        freq = st.slider("⏱️ 刷新间隔 (秒)", 0.1, 2.0, 0.5)
        ...
        st.button("⏹️ 强行停止", on_click=lambda: st.session_state.update({"is_live_trading": False}))
    with c_chart:
        met_ph = st.empty(); cht_ph = st.empty()
        if st.session_state.is_live_trading:
            stream = fetch_and_clean_data(format_ts_code(live_code), 'qfq', '20230101').tail(120).reset_index(drop=True)
            for i in range(20, len(stream)):
                if not st.session_state.is_live_trading: break
    ...
    st.markdown('<div class="glass-card"><h3 style="margin-bottom:0;">🛡️ 实验数据采集与多维审计中心</h3></div>', unsafe_allow_html=True)
    st.text_area("实时工作流终端", value="\n".join(st.session_state.sys_logs), height=350)
