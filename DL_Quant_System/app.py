    ...
    if raw_prompt := st.chat_input("向小吕布量化架构师发送军令..."):
        ...
        with chat_container:
            ...
            with st.chat_message("assistant"):
                ...
                1. **沙盒限制**：你只能使用 pandas (pd), numpy (np) 和 math。绝对禁止 `import talib` 或其他第三方库！
                2. **数据源字段**：传入的 df 包含的有效行情列名严格为：['Open', 'High', 'Low', 'Close', 'Volume']。
                3. **画图命名协议 (极其重要)**：
                   - 若要在【主图】叠加指标，列名必须以 `MAIN_` 开头。例如：`df['MAIN_MA20']`
                   - 若要在【新副图】显示指标，列名必须以 `SUB1_` (副图1) 或 `SUB2_` (副图2) 开头。
                ...
                for attempt in range(max_retries + 1):
                    ...
                    try:
                        ...
                        explanation = re.sub(r"`{3}python\s*.*?\s*`{3}", "", resp_clean,
                                             flags=re.DOTALL).strip().replace("【策略白话解析】", "").replace(
                            "【策略白话解析】:", "").replace("【策略白话解析】：", "").strip()
                        if explanation:
                ...
                with st.spinner("神经网络前向传播中..."):
                    try:
                        ...
                        class CNN_1D_Model(nn.Module):
                            def __init__(self, seq_len): super().__init__(); self.conv = nn.Conv1d(in_channels=1,
                                                                                                   out_channels=32,
                                                                                                   kernel_size=3,
                        ...
                        for m_idx, m_name in enumerate(model_choices):
                            ...
                            model = LSTM_Model() if m_name == "LSTM" else (
                                GRU_Model() if m_name == "GRU" else CNN_1D_Model(slen))
    ...
    with col_r:
        if st.session_state.dl_result:
            ...
            if len(res['models_used']) > 1:
                f_preds, h_preds, model_desc = np.mean(list(res['future'].values()), axis=0), np.mean(
                    list(res['preds'].values()), axis=0), f"LSTM/GRU/CNN 均值集成 ({len(res['models_used'])}模型)"
            else:
                f_preds, h_preds, model_desc = list(res['future'].values())[0], list(res['preds'].values())[0], \
                res['models_used'][0]
            act_diff, pred_diff = np.diff(actual_vals), np.diff(h_preds)
